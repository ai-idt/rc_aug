...
def randConv_linear(input_shape,n_layers=3,k=3):
    ...
    img_out = tf.keras.layers.Conv3D(filters=1,kernel_size=k,strides=1,padding="same",kernel_initializer=tf.keras.initializers.HeNormal())(img_)

    return tf.keras.Model(img_in,img_out)
